...
Focus on actionable improvements that address real customer pain points. Be specific and practical."""
    ...
    if len(configured_llms) >= 2:
        ...
        try:
            ...
            tasks = [call_llm(provider, key, prompt) for provider, key, _ in configured_llms]
...
Be specific and reference actual content from the posts when possible."""
    ...
    # If 2 or more LLMs are configured, call them in parallel and summarize
    if len(configured_llms) >= 2:
        logger.info(f"[Recommended Actions] {len(configured_llms)} LLMs configured, calling in parallel and summarizing")
        try:
            # Call all configured LLMs in parallel
            tasks = [call_llm_for_actions(provider, key, prompt) for provider, key, _ in configured_llms]
            results = await asyncio.gather(*tasks, return_exceptions=True)
            ...
            if not valid_results:
                ...
                if summarizer and summarizer_key:
                    summary_prompt = f"""You are analyzing recommended actions from multiple AI models. Below are the actions generated by {len(valid_results)} different AI models:

{chr(10).join([f'{name} actions:{chr(10)}{result}' for name, result in valid_results])}
...
- Include ROI insights only if you can provide meaningful estimates based on the data"""
    ...
    # If 2 or more LLMs are configured, call them in parallel and summarize
    if len(configured_llms) >= 2:
        logger.info(f"[Improvements Analysis] {len(configured_llms)} LLMs configured, calling in parallel and summarizing")
        try:
            # Call all configured LLMs in parallel
            tasks = [call_llm(provider, key, prompt) for provider, key, _ in configured_llms]
            results = await asyncio.gather(*tasks, return_exceptions=True)
            ...
            if not valid_results:
                ...
                if summarizer and summarizer_key:
                    summary_prompt = f"""You are analyzing product improvement insights from multiple AI models. Below are the analyses generated by {len(valid_results)} different AI models:

{chr(10).join([f'{name} analysis:{chr(10)}{result}' for name, result in valid_results])}
